...
//Rare exceptions to indicate a bug in the _venuefinder code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_venuefinder = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_venuefinder]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.venuefinder.module"
