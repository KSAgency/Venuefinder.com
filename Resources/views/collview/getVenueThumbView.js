function getVenueThumbView(venueObj, top, left, windowsArray, nextVenueId, isPageFlip, isSearchPage) {
	...
	if (isSearchPage != undefined && isSearchPage) {
		venueImageView = Ti.UI.createScrollView({
			width : '168',
			height : '92',
			top : '0',
			left : '0',
		...
		venueImageView = Ti.UI.createView({
			width : '168',
			height : '92',
			top : '0',
			left : '0',
	...
	venueView.addEventListener('click', function(e) {
		...
		var searchView = SearchView(null, currentWin, '', null, '', windowsArray, null, isSearchPage);
		...
		if (packageCode == 'GLD') {
			...
			var bronzePage = venueDetailPage.createDetailPage(venueID, nextVenueId, currentWin, windowsArray);
