function SearchView1(tabGroup, win, leftView, styleID, windowsArray) {
	...
	goButton.addEventListener('click', function() {
		...
		if (nameSearchField.value != null && nameSearchField.value != '') {
			...
			Ti.API.info('SELECT * FROM Venue JOIN VenueToVenueStyles ON Venue.VenueID = VenueToVenueStyles.VenueID WHERE VenueName LIKE "%'+nameSearchField.value+'%" AND VenueStyleID="'+styleID+'" AND (PackageCode="GLD" OR PackageCode="SIL" OR PackageCode="PRE") '+londonQuery+' ORDER BY VenueSort ASC');
			
			var sql = 'SELECT * FROM Venue JOIN VenueToVenueStyles ON Venue.VenueID = VenueToVenueStyles.VenueID WHERE VenueName LIKE "%'+nameSearchField.value+'%" AND VenueStyleID="'+styleID+'" AND (PackageCode="GLD" OR PackageCode="SIL" OR PackageCode="PRE") '+londonQuery+' ORDER BY VenueSort ASC';
